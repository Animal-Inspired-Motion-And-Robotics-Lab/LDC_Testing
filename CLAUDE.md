...
3. `crackDetectionCheck()` in [src/crack_detection.cpp](src/crack_detection.cpp) runs a two-step detector:
   ...
   2. **3D planarity check (t, Rp, L).** Picture the window as a curve in `(t, Rp, L)` space. A real crack lies mostly in the `t-L` plane — L bumps in time while Rp stays at the calibrated substrate baseline — whereas substrate motion tilts the curve so Rp moves together with L. We test this via the Pearson correlation `r = cov(Rp, L) / (std(Rp)·std(L))` over the rotated window and map `|r| ∈ [0, 1]` to a planar angle:
      ...
      `|r|≈0` (Rp and L independent) → `planar_angle ≈ π/2` → real crack. `|r|≈1` (Rp moves with L) → `planar_angle ≈ 0` → tilted plane, rejected. The candidate is confirmed when `planar_angle ≥ min_planar_angle_rad` and the fitted peak height is emitted as the crack magnitude. **Why correlation and not a raw `atan2(std(L), std(Rp))`:** L is in µH and Rp in Ω, on totally different magnitudes, so a raw std comparison is unit-biased toward "tilted" and rejects almost every candidate. Pearson `r` divides by the product of std deviations, so the units and scale cancel — the metric reflects the *shape* of the joint motion, not the magnitudes. (This replaced both the original chord-angle phase cone and a short-lived raw-std attempt.)
   3. **Stage 2b — rotated-Rp drift cap.** Pearson `r` has a known blind spot: when L peaks symmetrically across the window while Rp moves *monotonically* across it (the classic material-transition shape), the covariance terms on either side of the L peak cancel and `|r| ≈ 0`, so the planar check passes even though the curve is obviously tilted off the t-L plane. We catch that by characterizing the parabola's 3D plane orientation directly: for a parabolic L plus a linear-in-time Rp drift with slope `m`, the curve lies exactly in the plane with unit normal `(−m, 1, 0)/√(m²+1)`, and the angle between that plane and the t-L plane is `arctan(|m|)` — so the curve's tilt is captured entirely by `m`. We fit `m` by least squares over the window and reject when the predicted drift `|m·(N−1)|` exceeds `max_rp_drift_ohms`, in absolute ohms. The ohms framing lets the operator pick the threshold straight off the phase-space plot (typical: a real crack drifts a few Ω of sensor noise; a material transition drifts hundreds-to-thousands of Ω), sidestepping the unit-bias trap of comparing Ω directly to µH. Tuned via `crack_rp_drift` (default `500.0`; `0` disables the cap).
...
- Crack detector tuning lives in `crack_detection_config_t` constructed in `main.cpp::setup()`. These are also serial-tunable (`crack_window`, `crack_threshold`, `crack_r2`, `crack_planar`, `crack_rp_drift`, `crack_scale`), so changes to the struct defaults should be matched against the serial command bounds checks. Note that `min_planar_angle_rad` is clamped to `[0, π/2]` — it's the codomain of `atan2(non-negative, non-negative)`; `max_rp_drift_ohms` is clamped to `≥ 0` and treats `0` as disabled.
