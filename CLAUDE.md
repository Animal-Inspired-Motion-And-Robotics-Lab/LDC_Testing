...
3. `crackDetectionCheck()` in [src/crack_detection.cpp](src/crack_detection.cpp) runs a two-step detector:
   ...
   3. **Stage 2b — rotated-Rp range cap.** Pearson `r` has a known blind spot: when L peaks symmetrically across the window while Rp moves *monotonically* across it (the classic material-transition shape), the covariance terms on either side of the L peak cancel and `|r| ≈ 0`, so the planar check passes even though the curve is obviously tilted off the t-L plane. We catch that with a second, raw-magnitude test in the rotated frame: reject when `range(rotated Rp) > max_rp_l_range_ratio · range(rotated L)`. The unit-bias problem doesn't apply here because rotation has already aligned the substrate-baseline trend along Rp, so the two ranges are measured against the same "deviation from calibrated baseline" zero. Tuned via `crack_rp_ratio` (default `2.0`; `0` disables the cap).
...
- Crack detector tuning lives in `crack_detection_config_t` constructed in `main.cpp::setup()`. These are also serial-tunable (`crack_window`, `crack_threshold`, `crack_r2`, `crack_planar`, `crack_rp_ratio`, `crack_scale`), so changes to the struct defaults should be matched against the serial command bounds checks. Note that `min_planar_angle_rad` is clamped to `[0, π/2]` — it's the codomain of `atan2(non-negative, non-negative)`; `max_rp_l_range_ratio` is clamped to `≥ 0` and treats `0` as disabled.
